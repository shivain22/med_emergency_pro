...
public interface CommentRepository extends JpaRepository<Comment, Long>, JpaSpecificationExecutor<Comment> {
    ...
    @Query(
        value = "select distinct comment from Comment comment left join fetch comment.patient",
    ...
    @Query("select distinct comment from Comment comment left join fetch comment.patient")
    List<Comment> findAllWithToOneRelationships();

    @Query("select comment from Comment comment left join fetch comment.patient where comment.id =:id")
